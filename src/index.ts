...
// Catch errors and log them using dynamic import for ES Module support in CJS
import('electron-unhandled').then(({ default: unhandled }) => {
  unhandled({
...
if (is.linux()) {
  ...
  if (config.plugins.isEnabledSync('shortcuts')) {
    app.commandLine.appendSwitch('disable-features', 'MediaSessionService');
  }
}

if (config.get('options.proxy')) {
  const authProxyEnabled = config.plugins.isEnabledSync('auth-proxy-adapter');
...
function initTheme(win: BrowserWindow) {
  ...
  win.webContents.once('did-finish-load', () => {
    // Dev tools disabled for performance
    // if (is.dev()) {
...
async function createMainWindow() {
  ...
  const shouldShowApp = config.get('options.appVisible');
  ...
  if (windowPosition) {
    ...
    if (
      ...
      // Window is offscreen - silent for performance
      // if (is.dev()) {
      //   console.warn(
      //     LoggerPrefix,
  ...
  let mainReady = false;
  const showMain = (delay = 0) => {
    if (mainReady) return;
    mainReady = true;

    if (showTimeout) clearTimeout(showTimeout);

    if (win && shouldShowApp) {
      if (delay > 0) {
        setTimeout(() => {
          if (win && shouldShowApp) {
            win.show();
  ...
  const showTimeout = setTimeout(() => showMain(0), 8000);

  ipcMain.once('peard:player-api-loaded', () => showMain(1000));

  win.webContents.on('did-fail-load', () => showMain(0));
  win.webContents.on('did-start-navigation', (_event, url) => {
    if (url === urlToLoad || url.startsWith('http://localhost') || url.startsWith('http://127.0.0.1')) {
      return;
    }
    if (!url.includes('music.youtube.com') && !url.includes('youtube.com/signin')) {
      showMain(0);
    }
  });
  ...
  win.webContents.on('dom-ready', () => {
    // Disabled to allow custom HTML window control buttons when in-app menu is used
