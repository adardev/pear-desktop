...
import { allPlugins, pluginDefaults } from 'virtual:plugins';
...
export function isEnabledSync(plugin: string) {
  const pluginConfig = deepmerge(
    pluginDefaults[plugin] ?? { enabled: false },
