...
export const LyricsRenderer = () => {
  ...
  createEffect(() => {
    ...
    // Auto-switch to next provider when current one fails or has no data
    if (state === 'error' || (state === 'done' && !data?.lines && !data?.lyrics)) {
      const currentIdx = providerNames.indexOf(lyricsStore.provider);
      const nextIdx = currentIdx + 1;
      if (nextIdx < providerNames.length) {
        // Try next provider automatically
        setLyricsStore('provider', providerNames[nextIdx]);
        return;
      }
    }
