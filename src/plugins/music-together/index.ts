    ...
    async initMyProfile() {
      ...
      setTimeout(async () => {
        const renderer = await waitForElement<HTMLElement & { data: unknown }>(
          ...
          {
            maxRetry: 30, // 3 seconds max (30 * 100ms)
          },
        );
        if (!renderer) {
          console.warn('Music Together: Cannot find account (user might be logged out)');
          this.me = getDefaultProfile(this.connection?.id ?? '');
          // Close the menu if we opened it but didn't find the account header
          setTimeout(() => {
            const dropdown = document.querySelector<HTMLElement & { opened: boolean; close?: () => void }>('tp-yt-iron-dropdown');
            if (dropdown) {
              if (typeof dropdown.close === 'function') dropdown.close();
              else dropdown.opened = false;
            } else {
              accountButton.click();
            }
          }, 150);
        ...
        setTimeout(() => {
          const dropdown = renderer.closest<HTMLElement & { opened: boolean; close?: () => void }>('tp-yt-iron-dropdown, iron-dropdown');
          if (dropdown) {
